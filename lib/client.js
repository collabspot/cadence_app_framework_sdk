...
function rawPostMessage(client, msg, forceReady) {
  if (client.ready || forceReady) {
    ...
    client.on(Events.APP_REGISTERED, rawPostMessage.bind(null, client, msg));
  }
}
...
var Client = function(origin, appGuid) {
  ...
  this.on(Events.APP_REGISTERED, function(data) {
    this.ready = true;
    this._metadata = data.metadata;
    this._context = data.context;
  }, this);

  this.on(Events.CONTEXT_UPDATED, function(context) {
    this._context = context;
  }, this);

  this.postMessage(Events.IFRAME_HANDSHAKE, { version: version });
...
Client.prototype = {
  ...
  postMessage: function(name, data) {
    ...
    rawPostMessage(this, msg, name === Events.IFRAME_HANDSHAKE);
  },
  ...
  /// ```javascript
  /// var client = CAFClient.init();
  /// client.on(Events.APP_REGISTERED, function(e) {
  ///   // go nuts
  /// });
  /// ```
  ///
  ...
  on: function(name, handler, context) {
    if (typeof handler == 'function') {
      ...
      if (name !== Events.APP_REGISTERED) {
        this.postMessage(Events.IFRAME_ON + name);
      }
    }
  },
  ...
  /// var client = CAFClient.init();
  ///
  /// client.on(Events.APP_REGISTERED, function appRegistered(e) {
  ///   // do stuff then remove the handler
  ///   client.off(Events.APP_REGISTERED, appRegistered);
  /// });
  /// ```
  off: function(name, handler) {
    if (!this._messageHandlers[name]) { return false; }
    var index = this._messageHandlers[name].indexOf(handler);
    this.postMessage(Events.IFRAME_OFF + name);
  ...
  /// });
  ///
  /// client.has(Events.APP_REGISTERED, appRegistered); // true
  ...
  request: function(options) {
    var requestKey = Events.REQUEST + nextIdFor('request');
  ...
  metadata: function() {
    return new Promise(function(resolve) {
      if (this._metadata) {
        ...
        this.on(Events.APP_REGISTERED, function() {
          resolve(this._metadata);
        }.bind(this));
      }
    }.bind(this));
